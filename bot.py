...
INSTAGRAM_REVIEW_LINK = "https://www.instagram.com/golduc.tj?igsh=bHQxcmJpcHJiajZ4&utm_source=qr"
...
def send_welcome(message):
    ...
    bot.send_message(message.chat.id, "Салом! Пожалуйста, выберите язык / Лутфан забонро интихоб кунед:", reply_markup=markup)
...
def show_main_menu(chat_id, message_id, lang):
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        types.InlineKeyboardButton(LANGUAGES[lang]['btn_uc'], callback_data='menu_uc'),
        types.InlineKeyboardButton(LANGUAGES[lang]['btn_gold'], callback_data='menu_gold')
    )
    markup.add(types.InlineKeyboardButton(LANGUAGES[lang]['btn_lang'], callback_data='change_lang'))
...
def show_price_list(chat_id, message_id, item_type):
    ...
    if item_type == 'uc':
        ...
        wa_text = "Салом! Ба ман PUBG UC даркор аст." if lang == 'tj' else "Здравствуйте! Мне нужен PUBG UC."
    else:
        text = LANGUAGES[lang]['gold_price_list']
        wa_text = "Салом! Ба ман Gold даркор аст." if lang == 'tj' else "Здравствуйте! Мне нужен Gold."
    
    markup = types.InlineKeyboardMarkup(row_width=1)
    whatsapp_url = f"https://wa.me/{WHATSAPP_NUMBER}?text={urllib.parse.quote(wa_text)}"
    
    markup.add(types.InlineKeyboardButton(LANGUAGES[lang]['btn_whatsapp'], url=whatsapp_url))
    markup.add(types.InlineKeyboardButton(LANGUAGES[lang]['btn_reviews'], url=INSTAGRAM_REVIEW_LINK))
    markup.add(types.InlineKeyboardButton(LANGUAGES[lang]['btn_back'], callback_data='back_to_main'))
